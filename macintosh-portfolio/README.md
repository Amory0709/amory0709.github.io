...
初始最远位置、插入后的放大位置和手动近端分别控制。插入后的取景优先保留 **CRT 和已插入软盘**；电脑外壳和待选软盘可离开画面。桌面根据宽高比排列软盘；手机保留桌面单排场景及相机路径，仅通过横向滑动查看超出手机宽度的部分。

手机端电脑与软盘始终位于同一个 3D 场景，保留真实模型的倾斜角度、光照、标签和插入动画，不再生成独立缩略图栏。左右滑动场景可查看两侧软盘；选中作品时自动回到电脑位置，介绍 / 交互机位保持一致，并按可见手机宽度限制自动放大。

页面使用固定动态视口，不产生整页滚动条；手机 3D 场景、PDF、项目 iframe、署名弹层等内部内容保留独立滚动。支持 `prefers-reduced-motion`。
