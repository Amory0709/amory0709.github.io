...
手机端的待选软盘改为电脑下方的单排横向滑动栏；滑动查看更多作品，点击插入。介绍和交互网页状态下仍保留该栏，滑动不会控制电脑缩放。缩略图由实际 3D 模型及配置的标签生成，桌面软盘布局不变。

页面使用固定动态视口，不产生整页滚动条；软盘栏、PDF、项目 iframe、署名弹层等内部内容保留独立滚动。支持 `prefers-reduced-motion`。
